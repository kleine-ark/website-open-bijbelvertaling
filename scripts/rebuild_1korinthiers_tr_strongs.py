#!/usr/bin/env python3
"""Publiceer TR-woordnummers voor 1 Korinthiërs."""
...
def build(utr:Path,osis:Path,chapter_number:int,write:bool=False)->dict[str,int]:
 s=load_tr_chapter(utr,osis,chapter=chapter_number,osis_book="1Cor");p=ROOT/"data"/"1korinthiers"/f"{chapter_number}.json";d=json.loads(p.read_text(encoding="utf-8"));q={"book":"1korinthiers","chapter":chapter_number,"reviewed_through":len(s),"verses":{}}
 for v in d["verses"]:
  n=int(v["number"])
  if n not in s:v["grondtekst"]=[];v["woordnummers"]=[];continue
  t=s[n];ids=r(0,len(t)-1);a=v["text2026"];v["grondtekst"]=[{"woord":x["woord"],"strongs":x["display_strong"],"lemma_strongs":x["lemma_strong"],"morfologie":x["morphology"]}for x in t];v["woordnummers"]=[mapping(a,ids,t,n)];v["woordnummers"][0]["herkomst"]["referentie"]=f"1CO {chapter_number}:{n}";q["verses"][str(n)]=[{"tekst":a,"bronindices":ids,"reviewstatus":"handmatig_gecontroleerd"}]
 if write:
  p.write_text(json.dumps(d,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");(ROOT/"data"/"woordnummers-review"/f"1korinthiers-{chapter_number}.json").write_text(json.dumps(q,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");ip=ROOT/"data"/"woordnummers-inline"/"1korinthiers.json";i=json.loads(ip.read_text(encoding="utf-8"));i[str(chapter_number)]=q;ip.write_text(json.dumps(i,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")

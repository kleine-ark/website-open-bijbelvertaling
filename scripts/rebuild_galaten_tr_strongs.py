...
def build(utr:Path,osis:Path,c:int,write:bool=False)->dict[str,int]:
 s=load_tr_chapter(utr,osis,chapter=c,osis_book="Gal");p=ROOT/"data"/"galaten"/f"{c}.json";d=json.loads(p.read_text(encoding="utf8"));q={"book":"galaten","chapter":c,"reviewed_through":len(s),"verses":{}}
 for v in d["verses"]:
  n=int(v["number"])
 ...
 if write:
  p.write_text(json.dumps(d,ensure_ascii=False,indent=2)+"\n",encoding="utf8");(ROOT/"data"/"woordnummers-review"/f"galaten-{c}.json").write_text(json.dumps(q,ensure_ascii=False,indent=2)+"\n",encoding="utf8");ip=ROOT/"data"/"woordnummers-inline"/"galaten.json";i=json.loads(ip.read_text(encoding="utf8"));i[str(c)]=q;ip.write_text(json.dumps(i,ensure_ascii=False,indent=2)+"\n",encoding="utf8")

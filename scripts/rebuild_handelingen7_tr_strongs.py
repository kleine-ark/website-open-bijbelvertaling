#!/usr/bin/env python3
"""Publiceer de TR-woordnummers voor Handelingen 7."""
...
def build(utr:Path,osis:Path,write:bool=False)->dict[str,int]:
 s=load_tr_chapter(utr,osis,chapter=7,osis_book="Acts");p=ROOT/"data"/"handelingen"/"7.json";d=json.loads(p.read_text(encoding="utf-8"));q={"book":"handelingen","chapter":7,"reviewed_through":60,"verses":{}}
 for v in d["verses"]:
 ...
 if write:
  p.write_text(json.dumps(d,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");(ROOT/"data"/"woordnummers-review"/"handelingen-7.json").write_text(json.dumps(q,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");ip=ROOT/"data"/"woordnummers-inline"/"handelingen.json";i=json.loads(ip.read_text(encoding="utf-8"));i["7"]=q;ip.write_text(json.dumps(i,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")

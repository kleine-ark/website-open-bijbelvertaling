#!/usr/bin/env python3
"""Publiceer de TR-woordnummers voor Handelingen 3."""
...
def build(utr:Path,osis:Path,write:bool=False)->dict[str,int]:
 s=load_tr_chapter(utr,osis,chapter=3,osis_book="Acts");p=ROOT/"data"/"handelingen"/"3.json";d=json.loads(p.read_text(encoding="utf-8"));q={"book":"handelingen","chapter":3,"reviewed_through":26,"verses":{}}
 for v in d["verses"]:
 ...
 if write:
  p.write_text(json.dumps(d,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");(ROOT/"data"/"woordnummers-review"/"handelingen-3.json").write_text(json.dumps(q,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");ip=ROOT/"data"/"woordnummers-inline"/"handelingen.json";i=json.loads(ip.read_text(encoding="utf-8"));i["3"]=q;ip.write_text(json.dumps(i,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")

...
def build(u:Path,o:Path,w=False):
 s=load_tr_chapter(u,o,chapter=14,osis_book='Mark');p=R/'data'/'markus'/'14.json';d=json.loads(p.read_text(encoding='utf8'));q={'book':'markus','chapter':14,'reviewed_through':72,'verses':{}}
 for v in d['verses']:
 ...
 if w:
  p.write_text(json.dumps(d,ensure_ascii=False,indent=2)+'\n',encoding='utf8');(R/'data'/'woordnummers-review'/'markus-14.json').write_text(json.dumps(q,ensure_ascii=False,indent=2)+'\n',encoding='utf8');ip=R/'data'/'woordnummers-inline'/'markus.json';i=json.loads(ip.read_text(encoding='utf8'));i['chapters']['14']={str(v['number']):v['woordnummers'] for v in d['verses']};ip.write_text(json.dumps(i,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
 return {'verses':72,'tokens':sum(len(x) for x in s.values())}

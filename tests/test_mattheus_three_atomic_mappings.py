...
EXPECTED_GROUND_SHA = "FB640FD116231AE5F489C8BB987730E7EC2C78843C0849D8796FF41D4FDE9D00"
...
def test_mattheus_three_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "mattheus" / "3.json")
    review_path = ROOT / "data" / "woordnummers-review" / "mattheus-3.json"
    ...
    assert mapping_count == 335
    assert link_count == 335


def test_mattheus_three_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "mattheus" / "3.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 335
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 335
    assert not any(

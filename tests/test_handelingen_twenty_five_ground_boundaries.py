...
def _chapter():
    return json.loads((ROOT / "data" / "handelingen" / "25.json").read_text(encoding="utf-8"))


def _review():
    return json.loads(
        (ROOT / "data" / "woordnummers-review" / "handelingen-25.json").read_text(
            encoding="utf-8"
        )
    )


def test_handelingen_vijfentwintig_behoudt_alle_grondtokens_en_verzen():
    chapter = _chapter()
    assert [verse["number"] for verse in chapter["verses"]] == list(range(1, 28))
    assert sum(len(verse.get("grondtekst", [])) for verse in chapter["verses"]) == 540


def test_handelingen_vijfentwintig_review_dekt_iedere_grondindex_exact_eenmaal():
    chapter = _chapter()
...
def test_handelingen_vijfentwintig_publiceert_atomische_koppelingen():

...
GROUND_SHA = "6FA4DEEA694B03203741987C67A0D21DD700F1B0AEDFA1C112DEF5DF35AEE9D5"
...
def test_1korinthiers_15_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "1korinthiers" / "15.json")
    review_path = ROOT / "data" / "woordnummers-review" / "1korinthiers-15.json"
    ...
    assert [verse["verse"] for verse in book["verses"]] == list(range(1, 59))
    ...
    assert link_count == 853


def test_1korinthiers_15_publishes_all_853_links_at_atomic_targets():
    chapter = _load(ROOT / "data" / "1korinthiers" / "15.json")
    inline = _load(ROOT / "data" / "woordnummers-inline" / "1korinthiers.json")
    inline_verses = inline["chapters"]["15"]
...
def test_1korinthiers_15_keeps_every_runtime_target_unambiguous():
    chapter = _load(ROOT / "data" / "1korinthiers" / "15.json")
    for verse in chapter["verses"]:

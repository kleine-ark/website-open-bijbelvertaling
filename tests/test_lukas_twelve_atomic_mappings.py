...
EXPECTED_GROUND_SHA = "B55CE9B78F707D92BFE3C0723F6E5D95A0515875D87C314C8C2965692EA077AF"
...
def test_lukas_twelve_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "12.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-12.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 60))
    ...
    assert mapping_count == 826
    assert link_count == 1058


def test_lukas_twelve_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "12.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 826
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 1058
    assert not any(

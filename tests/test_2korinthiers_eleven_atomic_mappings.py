...
CHAPTER = ROOT / "data" / "2korinthiers" / "11.json"
REVIEW = ROOT / "data" / "woordnummers-review" / "2korinthiers-11.json"
...
def test_ground_boundary_matches_the_visible_verses():
    chapter = _data(CHAPTER)
    verses = {verse["number"]: verse for verse in chapter["verses"]}
    assert sum(len(verse["grondtekst"]) for verse in chapter["verses"]) == 502
    assert len(verses[8]["grondtekst"]) == 18
    assert [token["woord"] for token in verses[8]["grondtekst"][-9:]] == [
        "και",
        "παρων",
        "προς",
        "υμας",
    ...
    assert len(verses[9]["grondtekst"]) == 19
    assert verses[9]["grondtekst"][0]["woord"] == "το"
...
def test_review_is_atomic_and_has_no_proposals():
    ...
    for verse in verses:
        for mapping in verse["mappings"]:
            ...
            assert len(mapping["grondindices"]) <= 2
            assert not mapping["tekst"] or len(mapping["tekst"].split()) <= 3


def test_repeated_targets_and_empty_anchors_are_explicit():
    chapter, review = _data(CHAPTER), _data(REVIEW)
...
def test_projected_chapter_has_all_502_atomic_links():
    chapter = _data(CHAPTER)
    inline = _data(INLINE)["chapters"]["11"]
    assert sum(
        len(mapping["strongs"])

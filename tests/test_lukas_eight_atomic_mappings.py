...
EXPECTED_GROUND_SHA = "E0642C6325D2236625AFF597DA603404C6EC9CE4163266A9DE09AFE00A7D4F41"
...
def test_lukas_eight_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "8.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-8.json"
    ...
    assert mapping_count == 869
    assert link_count == 1116


def test_lukas_eight_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "8.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 869
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 1116
    assert not any(

...
CHAPTER = ROOT / "data" / "2korinthiers" / "9.json"
REVIEW = ROOT / "data" / "woordnummers-review" / "2korinthiers-9.json"
...
def test_review_is_fully_atomic_and_has_no_proposals():
    ...
    assert len(verses) == 15
    assert sum(len(verse["mappings"]) for verse in verses) == 287
    assert sum(
        len(mapping["grondindices"])
        for verse in verses
        for mapping in verse["mappings"]
    ) == 287
    assert sum(len(verse.get("bronafwijkingen", [])) for verse in verses) == 10
    for verse in verses:
        for mapping in verse["mappings"]:
...
def test_all_repeated_targets_and_empty_anchors_are_explicit():
    ...
    assert [
        mapping["voorkomen"]
        for mapping in verses[6]["mappings"]
        if mapping["tekst"].casefold() == "karig"
    ] == [1, 2]
    assert [
        mapping["voorkomen"]
        for mapping in verses[14]["mappings"]
        if mapping["tekst"].casefold() == "u"
    ] == [1, 2, 3]


def test_projected_chapter_has_all_287_atomic_links():
    chapter, review = _data(CHAPTER), _data(REVIEW)
    inline = _data(INLINE)["chapters"]["9"]
    reviewed = review["books"][0]["verses"]
    assert sum(len(verse["mappings"]) for verse in reviewed) == 287
    assert sum(len(verse["woordnummers"]) for verse in chapter["verses"]) == 287
    assert sum(
        len(mapping["strongs"])
        for verse in chapter["verses"]
        for mapping in verse["woordnummers"]
    ) == 287
    assert sum(len(verse) for verse in inline.values()) == 287
    assert sum(
        len(mapping["strongs"])

...
EXPECTED_GROUND_SHA = "43873B1F60D143DD609503C65612C0DB2FB3C5454E4AD23C64B0B70703638799"
...
def test_lukas_nineteen_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "19.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-19.json"
    ...
    assert mapping_count == 674
    assert link_count == 768


def test_lukas_nineteen_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "19.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 674
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 768
    assert not any(

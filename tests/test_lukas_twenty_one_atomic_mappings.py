...
EXPECTED_GROUND_SHA = "34DA5ACAB7EF93714FE0437D6FAF0D6F0E6452FAC513AA41CE808208E30B2910"
...
def test_lukas_twenty_one_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "21.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-21.json"
    ...
    assert link_count == 593


def test_lukas_twenty_one_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "21.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 477
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 593
    assert not any(

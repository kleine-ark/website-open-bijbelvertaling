...
CHAPTER = ROOT / "data" / "galaten" / "4.json"
REVIEW = ROOT / "data" / "woordnummers-review" / "galaten-4.json"
GROUND_SHA = "C07664CB4FC5FA1428BF0B6097D2EBA9B8EF70F835B7A865A57317DC8522BC51"
...
def test_galatians_four_pins_and_complete_ground_coverage():
    ...
    assert sorted(verses) == list(range(1, 32))
    assert sorted(reviewed) == list(range(1, 32))
...
def test_galatians_four_review_is_atomic_manual_and_reachable():
    ...
    assert sum(len(mapping["grondindices"]) for mapping in mappings) == 451
...
def test_galatians_four_keeps_tr_readings_and_semantic_source_order():
    _, _, verses, reviewed = _data()
    ...
    assert mapping(11, "dat ik niet enigszins")["bronindices"] == [2, 6]
    assert mapping(14, "mijn", 2)["bronindices"] == []
    assert mapping(26, "alle")["bronindices"] == []
    assert mapping(31, "Zo dan")["bronindices"] == [0]

    assert [verses[11]["grondtekst"][index]["strongs"] for index in mapping(11, "dat ik niet enigszins")["grondindices"]] == ["G3381"]
    assert [verses[14]["grondtekst"][index]["strongs"] for index in mapping(14, "mijn", 2)["grondindices"]] == ["G1473"]
    assert [verses[26]["grondtekst"][index]["strongs"] for index in mapping(26, "alle")["grondindices"]] == ["G3956"]
    assert [verses[31]["grondtekst"][index]["strongs"] for index in mapping(31, "Zo dan")["grondindices"]] == ["G686"]


def test_galatians_four_publishes_every_link_at_its_atomic_target():
    ...
    assert embedded_mappings == projected_mappings == 450
    assert embedded_links == projected_links == 451

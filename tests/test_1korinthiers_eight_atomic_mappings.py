...
GROUND_SHA = "C8F62024A4A983D05760C5520C8E5CA64CA4F3F14E06AA681B6D49B5B8178877"
...
def test_1korinthiers_8_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "1korinthiers" / "8.json")
    review_path = ROOT / "data" / "woordnummers-review" / "1korinthiers-8.json"
    ...
    assert mapping_count == 228
    assert link_count == 228


def test_1korinthiers_8_publishes_all_228_links_at_atomic_targets():
    chapter = _load(ROOT / "data" / "1korinthiers" / "8.json")
    inline = _load(ROOT / "data" / "woordnummers-inline" / "1korinthiers.json")
    inline_verses = inline["chapters"]["8"]
    ...
    assert sum(len(verse["woordnummers"]) for verse in chapter["verses"]) == 228
    assert sum(len(items) for items in inline_verses.values()) == 228


def test_1korinthiers_8_preserves_the_documented_tr_variants():
    chapter = _load(ROOT / "data" / "1korinthiers" / "8.json")
    review = _load(ROOT / "data" / "woordnummers-review" / "1korinthiers-8.json")
    ...
    assert strongs_for(2, "te weten") == ["G1492"]
    assert strongs_for(2, "nog") == ["G3764"]
    assert strongs_for(7, "geweten", 1) == ["G4893"]
    assert strongs_for(8, "want") == ["G1063"]
    assert strongs_for(9, "zwak zijn") == ["G770"]
    assert strongs_for(11, "En") == ["G2532"]
    assert strongs_for(12, "zo") == ["G3779"]

...
CHAPTER = ROOT / "data" / "2korinthiers" / "4.json"
REVIEW = ROOT / "data" / "woordnummers-review" / "2korinthiers-4.json"
...
def test_review_pins_and_ground_coverage_are_exact():
    ...
    for verse in book["verses"]:
        ...
        assert sorted(indices) == list(
            range(len(local[verse["verse"]]["grondtekst"]))
        )
...
def test_projected_chapter_has_all_324_links_at_atomic_anchors():
    chapter, review = _data(CHAPTER), _data(REVIEW)
    inline = _data(INLINE)["chapters"]["4"]
    reviewed = review["books"][0]["verses"]
    assert sum(len(verse["mappings"]) for verse in reviewed) == 226
    assert sum(
        len(mapping["grondindices"])
        for verse in reviewed
        for mapping in verse["mappings"]
    ) == 324
    assert sum(len(verse["woordnummers"]) for verse in chapter["verses"]) == 226
    assert sum(
        len(mapping["strongs"])
        for verse in chapter["verses"]
        for mapping in verse["woordnummers"]
    ) == 324
    assert sum(len(verse) for verse in inline.values()) == 226
    assert sum(
        len(mapping["strongs"])

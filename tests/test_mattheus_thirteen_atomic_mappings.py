...
EXPECTED_GROUND_SHA = "011FC6BA9C5C265E9709AFFF3DC97F9C631E7F277910ADC0BAAF4EB7AA5AD5D2"
...
def test_mattheus_thirteen_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "mattheus" / "13.json")
    review_path = ROOT / "data" / "woordnummers-review" / "mattheus-13.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 59))
    ...
    assert mapping_count == 1086
    assert link_count == 1100


def test_mattheus_thirteen_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "mattheus" / "13.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 1086
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 1100
    assert not any(

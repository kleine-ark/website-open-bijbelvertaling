...
EXPECTED_GROUND_SHA = "E987E8CB82683E5F2ABF75FA1A01EB25BDAF017F61A0D16EE63B280365578211"
...
def test_lukas_seven_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "7.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-7.json"
    ...
    assert mapping_count == 743
    assert link_count == 916


def test_lukas_seven_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "7.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 743
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 916
    assert not any(

...
GROUND_SHA = "49AED089E1C8BEE734480509956AE37D69DFEC060928079978341129E3CF8EEB"
...
def test_1korinthiers_11_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "1korinthiers" / "11.json")
    review_path = ROOT / "data" / "woordnummers-review" / "1korinthiers-11.json"
    ...
    assert [verse["verse"] for verse in book["verses"]] == list(range(1, 35))
    ...
    assert link_count == 539


def test_1korinthiers_11_publishes_all_539_links_at_atomic_targets():
    chapter = _load(ROOT / "data" / "1korinthiers" / "11.json")
    inline = _load(ROOT / "data" / "woordnummers-inline" / "1korinthiers.json")
    inline_verses = inline["chapters"]["11"]
...
def test_1korinthiers_11_keeps_every_runtime_target_unambiguous():
    chapter = _load(ROOT / "data" / "1korinthiers" / "11.json")
    for verse in chapter["verses"]:

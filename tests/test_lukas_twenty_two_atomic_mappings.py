...
EXPECTED_GROUND_SHA = "71A0864FF59C8F18C72F79A5EEBC6824B488DF63264CA037B3CB718C369F1CDC"
...
def test_lukas_twenty_two_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "22.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-22.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 72))
    ...
    assert mapping_count == 925
    assert link_count == 1115


def test_lukas_twenty_two_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "22.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 925
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 1115
    assert not any(

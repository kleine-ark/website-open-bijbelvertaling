...
class OpvReaderTests(unittest.TestCase):
    ...
    def test_opv_toont_buiten_gepubliceerde_dekking_een_explicitiete_melding(self):
        ...
        try:
            page.goto(
                f"{self.base_url}/index.html?editie=nl-opv#exodus/29",
                wait_until="domcontentloaded",
            )
            unavailable = page.locator(".translation-unavailable")
            unavailable.wait_for()
            self.assertIn("Exodus is niet beschikbaar", unavailable.inner_text())
    ...
    def test_ov_audio_wordt_gewist_bij_onbeschikbare_opv_en_herstelt_via_link(self):
        ...
        try:
            page.goto(f"{self.base_url}/index.html#exodus/29", wait_until="domcontentloaded")
            ...
            self.assertIn("audio/exodus/29-m.mp3", page.locator("#audio-el").get_attribute("src"))
            ...
            self.assertFalse(
                any(url.endswith("/data/edities/opv/chapters/exodus/29.json") for url in observed["requests"])
            )

            restore = unavailable.locator("a")
            self.assertIn("?editie=nl-ov#exodus/29", restore.get_attribute("href"))
    ...
    def test_opv_prefetch_respecteert_werkelijk_gepubliceerde_hoofdstukken(self):
        ...
        try:
            ...
            self.assertFalse(any(url.endswith("/exodus/6.json") for url in opv_requests))
            observed["requests"].clear()
            page.goto(

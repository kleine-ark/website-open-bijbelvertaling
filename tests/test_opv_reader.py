...
class OpvReaderTests(unittest.TestCase):
    ...
    def test_opv_toont_buiten_gepubliceerde_dekking_een_explicitiete_melding(self):
        ...
        try:
            page.goto(
                f"{self.base_url}/index.html?editie=nl-opv#genesis/11",
                wait_until="domcontentloaded",
            )
    ...
    def test_ov_audio_wordt_gewist_bij_onbeschikbare_opv_en_herstelt_via_link(self):
        ...
        try:
            page.goto(f"{self.base_url}/index.html#genesis/11", wait_until="domcontentloaded")
            ...
            self.assertIn("audio/genesis/11-m.mp3", page.locator("#audio-el").get_attribute("src"))
            ...
            self.assertFalse(
                any(url.endswith("/data/edities/opv/chapters/genesis/11.json") for url in observed["requests"])
            )

            restore = unavailable.locator("a")
            self.assertIn("?editie=nl-ov#genesis/11", restore.get_attribute("href"))
    ...
    def test_opv_prefetch_respecteert_werkelijk_gepubliceerde_hoofdstukken(self):
        ...
        try:
            ...
            self.assertFalse(any(url.endswith("/genesis/11.json") for url in opv_requests))
            observed["requests"].clear()
            page.goto(

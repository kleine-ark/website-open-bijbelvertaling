...
class OpvReaderTests(unittest.TestCase):
    ...
    def test_opv_toont_buiten_gepubliceerde_dekking_een_explicitiete_melding(self):
        ...
        try:
            page.goto(
                f"{self.base_url}/index.html?editie=nl-opv#exodus/11",
                wait_until="domcontentloaded",
            )
    ...
    def test_ov_audio_wordt_gewist_bij_onbeschikbare_opv_en_herstelt_via_link(self):
        ...
        try:
            page.goto(f"{self.base_url}/index.html#exodus/11", wait_until="domcontentloaded")
            ...
            self.assertIn("audio/exodus/11-m.mp3", page.locator("#audio-el").get_attribute("src"))
            ...
            self.assertFalse(
                any(url.endswith("/data/edities/opv/chapters/exodus/11.json") for url in observed["requests"])
            )

            restore = unavailable.locator("a")
            self.assertIn("?editie=nl-ov#exodus/11", restore.get_attribute("href"))

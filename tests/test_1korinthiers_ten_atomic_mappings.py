...
GROUND_SHA = "532D6F6357DA37DDCD24548A4B7A32F1FA2F677CE1718BAFE1C8AB5E10C81D21"
...
def test_1korinthiers_10_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "1korinthiers" / "10.json")
    review_path = ROOT / "data" / "woordnummers-review" / "1korinthiers-10.json"
    ...
    assert [verse["verse"] for verse in book["verses"]] == list(range(1, 34))
    ...
    assert link_count == 484


def test_1korinthiers_10_publishes_all_484_links_at_atomic_targets():
    chapter = _load(ROOT / "data" / "1korinthiers" / "10.json")
    inline = _load(ROOT / "data" / "woordnummers-inline" / "1korinthiers.json")
    inline_verses = inline["chapters"]["10"]
...
def test_1korinthiers_10_keeps_every_runtime_target_unambiguous():
    chapter = _load(ROOT / "data" / "1korinthiers" / "10.json")
    for verse in chapter["verses"]:

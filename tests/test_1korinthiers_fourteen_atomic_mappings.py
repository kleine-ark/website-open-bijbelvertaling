...
GROUND_SHA = "931CC8BB218636B59FCD22412A0A6C87DE5F01E5780556E64DB76EB48F2768DC"
...
def test_1korinthiers_14_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "1korinthiers" / "14.json")
    review_path = ROOT / "data" / "woordnummers-review" / "1korinthiers-14.json"
    ...
    assert link_count == 615


def test_1korinthiers_14_publishes_all_615_links_at_atomic_targets():
    chapter = _load(ROOT / "data" / "1korinthiers" / "14.json")
    inline = _load(ROOT / "data" / "woordnummers-inline" / "1korinthiers.json")
    inline_verses = inline["chapters"]["14"]
...
def test_1korinthiers_14_keeps_every_runtime_target_unambiguous():
    chapter = _load(ROOT / "data" / "1korinthiers" / "14.json")
    for verse in chapter["verses"]:

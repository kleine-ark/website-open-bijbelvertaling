...
EXPECTED_GROUND_SHA = "C76C68909566A428FCDEDFC100C6FF5EE98B6B4551E6D806434002BFF10E21F0"
...
def test_lukas_twenty_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "20.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-20.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 48))
...
def test_lukas_twenty_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "20.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 591
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 723
    assert not any(

...
EXPECTED_GROUND_SHA = "61190501DFFFC95E8C42F3DDB90645BB9E18CD101BA36ED61AE01613D514C114"
...
def _whole_word_occurrences(text: str, target: str):
    return list(re.finditer(rf"(?<!\w){re.escape(target)}(?!\w)", text, re.IGNORECASE | re.UNICODE))


def test_lukas_six_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "6.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-6.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 50))
    ...
    assert mapping_count == 792
    assert link_count == 957

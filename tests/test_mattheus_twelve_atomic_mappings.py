...
EXPECTED_GROUND_SHA = "E6EF5CB17F83B3D13A330F84687BB959C7EC8D166E9B0FF70DFF1D9BF43C643F"
...
def test_mattheus_twelve_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "mattheus" / "12.json")
    ...
    assert mapping_count == 921
    assert link_count == 922


def test_mattheus_twelve_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "mattheus" / "12.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 921
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 922
    assert not any(

...
EXPECTED_GROUND_SHA = "6FDA8E52F0D978BDBE62CF8CDEC2CBC6A91AFB97ECC16FD1C2D7AFB29C576596"
...
def test_markus_eleven_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "markus" / "11.json")
    review_path = ROOT / "data" / "woordnummers-review" / "markus-11.json"
    ...
    assert mapping_count == 591
    assert link_count == 598


def test_markus_eleven_tr_only_verse_remains_explicit():
    review = _load(ROOT / "data" / "woordnummers-review" / "markus-11.json")
    record = next(item for item in review["books"][0]["verses"] if item["verse"] == 26)
    missing_from_guide = [
        item for item in record["gidsafwijkingen"] if not item["bronindices"]
    ]

    assert len(missing_from_guide) == 1
    assert missing_from_guide[0]["grondindices"] == list(range(17))
    assert len(missing_from_guide[0]["grondtekst_strongs"]) == 17

...
GROUND_SHA = "1E5AA92927EFAA4904FD0038753A4247F87FA0DEF3C3DC64FF8B08DF938EB69F"
...
def test_1korinthiers_4_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "1korinthiers" / "4.json")
    review_path = ROOT / "data" / "woordnummers-review" / "1korinthiers-4.json"
...
def test_1korinthiers_4_keeps_local_tr_readings_at_their_words():
    chapter = _load(ROOT / "data" / "1korinthiers" / "4.json")
    review = _load(ROOT / "data" / "woordnummers-review" / "1korinthiers-4.json")
    ...
    assert strongs_for(2, "En") == ["G3739", "G1161"]
    assert strongs_for(6, "te gevoelen") == ["G5426"]
    assert strongs_for(13, "worden gelasterd") == ["G987"]
    assert strongs_for(17, "Christus") == ["G5547"]


def test_1korinthiers_4_publishes_every_link_at_an_atomic_target():
    chapter = _load(ROOT / "data" / "1korinthiers" / "4.json")
    ...
    assert sum(len(verse["woordnummers"]) for verse in chapter["verses"]) == 342
    assert sum(len(items) for items in inline_verses.values()) == 342

...
EXPECTED_GROUND_SHA = "19A600D2CBC57406B479F87408F6A280509330E06BA9E59F28600AA0F55F5178"
...
def test_lukas_nine_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "9.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-9.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 63))
    ...
    assert mapping_count == 935
    assert link_count == 1199


def test_lukas_nine_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "9.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 935
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 1199
    assert not any(

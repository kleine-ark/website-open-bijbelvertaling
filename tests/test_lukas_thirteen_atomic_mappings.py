...
EXPECTED_GROUND_SHA = "F2FBA06F878FCE2E690339D80455AC005E5CD94FEDC541F6935CA7DC0F71907F"
...
def test_lukas_thirteen_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "13.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-13.json"
    ...
    assert mapping_count == 506
    assert link_count == 674


def test_lukas_thirteen_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "13.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 506
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 674
    assert not any(

...
EXPECTED_GROUND_SHA = "405675AA3D31152800F7572C829475A980C49F949E42CBEBE51D71066383EA8C"
...
def test_mattheus_six_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "mattheus" / "6.json")
    review_path = ROOT / "data" / "woordnummers-review" / "mattheus-6.json"
    ...
    assert link_count == 684


def test_mattheus_six_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "mattheus" / "6.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 684
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 684
    assert not any(

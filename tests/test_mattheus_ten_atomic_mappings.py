...
EXPECTED_GROUND_SHA = "AABE8CA351BD30117EEC38437E837D76F1559AF6F22306F10E9F256DD7C7FFE4"
...
def test_mattheus_ten_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "mattheus" / "10.json")
    review_path = ROOT / "data" / "woordnummers-review" / "mattheus-10.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 43))
    ...
    assert mapping_count == 722
    assert link_count == 722


def test_mattheus_ten_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "mattheus" / "10.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 722
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 722
    assert not any(

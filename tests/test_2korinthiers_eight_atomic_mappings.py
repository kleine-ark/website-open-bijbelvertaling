...
CHAPTER = ROOT / "data" / "2korinthiers" / "8.json"
REVIEW = ROOT / "data" / "woordnummers-review" / "2korinthiers-8.json"
...
def test_review_is_fully_atomic_and_has_no_proposals():
    ...
    assert len(verses) == 24
    assert sum(len(verse["mappings"]) for verse in verses) == 414
    assert sum(
...
def test_split_source_verse_and_repeated_targets_are_explicit():
    review = _data(REVIEW)
    verses = {verse["verse"]: verse for verse in review["books"][0]["verses"]}
    assert [
        mapping.get("source_verse") for mapping in verses[14]["mappings"][:3]
    ] == [13, 13, 13]
    assert [
        mapping.get("voorkomen", 1)
        for mapping in verses[14]["mappings"]
        if mapping["tekst"] == "gelijkheid"
    ] == [1, 2]
    assert [
        mapping["voorkomen"]
        for mapping in verses[14]["mappings"]
        if mapping["tekst"] == "opdat"
    ] == [2, 3]
    assert [
        mapping["voorkomen"]
        for mapping in verses[14]["mappings"]
        if mapping["tekst"] == "zij"
    ] == [2]
    article = next(
        mapping
        for mapping in verses[19]["mappings"]
        if mapping["grondindices"] == [12]
    )
    assert article["tekst"] == ""
    assert article["anker"] == "deze gave"
    assert [
        mapping.get("voorkomen", 1)
        for mapping in verses[21]["mappings"]
        if mapping["tekst"] == "voor"
    ] == [1, 2]


def test_projected_chapter_has_all_414_atomic_links():
    chapter, review = _data(CHAPTER), _data(REVIEW)
    inline = _data(INLINE)["chapters"]["8"]
    reviewed = review["books"][0]["verses"]
    assert sum(len(verse["mappings"]) for verse in reviewed) == 414
    assert sum(len(verse["woordnummers"]) for verse in chapter["verses"]) == 414
    assert sum(
        len(mapping["strongs"])
        for verse in chapter["verses"]
        for mapping in verse["woordnummers"]
    ) == 414
    assert sum(len(verse) for verse in inline.values()) == 414
    assert sum(
        len(mapping["strongs"])

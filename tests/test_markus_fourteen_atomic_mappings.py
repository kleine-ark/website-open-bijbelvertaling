...
EXPECTED_GROUND_SHA = "15B2A87A1916C0CAC76C27F493300607571B2D8BEF2AE75A887B83DF8EBCFD8A"
...
def test_markus_fourteen_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "markus" / "14.json")
    review_path = ROOT / "data" / "woordnummers-review" / "markus-14.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 73))
    ...
    assert mapping_count == 1230
    assert link_count == 1235


def test_markus_fourteen_no_long_whole_verse_mapping_remains():
    review = _load(ROOT / "data" / "woordnummers-review" / "markus-14.json")
    for record in review["books"][0]["verses"]:
        assert all(len(mapping["grondindices"]) <= 3 for mapping in record["mappings"])
        assert all(len(mapping.get("tekst", "").split()) <= 4 for mapping in record["mappings"])

...
EXPECTED_GROUND_SHA = "3371F9934063095BAE2773CFBEB4E66B759FF1DECD6B31FAFBE46F3EDAFE3C09"
...
def test_lukas_eighteen_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "18.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-18.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 44))
    ...
    assert mapping_count == 588
    assert link_count == 689


def test_lukas_eighteen_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "18.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 588
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 689
    assert not any(

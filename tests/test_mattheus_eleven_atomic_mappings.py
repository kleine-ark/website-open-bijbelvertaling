...
EXPECTED_GROUND_SHA = "4081279D6EEC2450361C51B7FEBBA6168A1DF0FED3AC9252FAC8FEDC973D961A"
...
def test_mattheus_eleven_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "mattheus" / "11.json")
    review_path = ROOT / "data" / "woordnummers-review" / "mattheus-11.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 31))
    ...
    assert mapping_count == 498
    assert link_count == 498


def test_mattheus_eleven_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "mattheus" / "11.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 498
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 498
    assert not any(

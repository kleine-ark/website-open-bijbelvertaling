...
EXPECTED_GROUND_SHA = "290508CA262D015141A04F363776507E8BE231BACABCE3C429931CC91587A47C"
...
def test_lukas_seventeen_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "17.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-17.json"
    ...
    assert mapping_count == 507
    assert link_count == 595


def test_lukas_seventeen_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "17.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 507
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 595
    assert not any(

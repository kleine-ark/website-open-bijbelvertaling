...
EXPECTED_GROUND_SHA = "78FACB2747CC9CA718328B161B4B23A8748328E9F2CF4968D2A67C4476D28DCD"
...
def test_mattheus_seven_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "mattheus" / "7.json")
    review_path = ROOT / "data" / "woordnummers-review" / "mattheus-7.json"
    ...
    for record in book["verses"]:
        ...
        for index, mapping in enumerate(mappings):
            ...
            if target:
                ...
                occurrences = _whole_word_occurrences(verse["text2026"], mapping["anker"])
            ...
            key = (
                record["verse"], target.casefold(), mapping.get("anker", "").casefold(),
                occurrence, strong,
            )
            assert key not in runtime_keys, key
            runtime_keys.add(key)

    assert mapping_count == 516
    assert link_count == 516


def test_mattheus_seven_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "mattheus" / "7.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 516
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 516
    assert not any(

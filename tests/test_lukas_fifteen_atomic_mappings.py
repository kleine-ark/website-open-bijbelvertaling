...
EXPECTED_GROUND_SHA = "235EECE15A73394E3763848738E7EDEB17F4AD409F9C4D5BCA986B2FB697670B"
...
def test_lukas_fifteen_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "15.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-15.json"
    ...
    assert mapping_count == 419
    assert link_count == 563


def test_lukas_fifteen_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "15.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 419
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 563
    assert not any(

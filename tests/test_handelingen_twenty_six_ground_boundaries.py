...
def _chapter():
    return json.loads((ROOT / "data" / "handelingen" / "26.json").read_text(encoding="utf-8"))


def _review():
    return json.loads(
        (ROOT / "data" / "woordnummers-review" / "handelingen-26.json").read_text(
            encoding="utf-8"
        )
    )


def test_handelingen_zesentwintig_behoudt_alle_grondtokens_en_verzen():
    chapter = _chapter()
    assert [verse["number"] for verse in chapter["verses"]] == list(range(1, 33))
    assert sum(len(verse.get("grondtekst", [])) for verse in chapter["verses"]) == 597


def test_handelingen_zesentwintig_review_dekt_iedere_grondindex_exact_eenmaal():
    chapter = _chapter()
    review_verses = _review()["books"][0]["verses"]
    assert [verse["verse"] for verse in review_verses] == list(range(1, 33))
...
def test_handelingen_zesentwintig_publiceert_atomische_koppelingen():

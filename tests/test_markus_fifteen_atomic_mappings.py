...
def test_markus_fifteen_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "markus" / "15.json")
    review_path = ROOT / "data" / "woordnummers-review" / "markus-15.json"
    review = _load(review_path)
    book = review["books"][0]
    verses = {int(verse["number"]): verse for verse in chapter["verses"]}
    expected_ground_sha = _grondtekst_sha256(chapter)
    ...
    assert mapping_count == 693
    assert link_count == 693

...
EXPECTED_GROUND_SHA = "DA2E2664CD648D54DEF5E2EF196C4ACF2BDC0076587BF58C1F411190C8F2DDB1"
...
def test_lukas_eleven_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "11.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-11.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 55))
    ...
    assert mapping_count == 810
    assert link_count == 1029


def test_lukas_eleven_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "11.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 810
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 1029
    assert not any(

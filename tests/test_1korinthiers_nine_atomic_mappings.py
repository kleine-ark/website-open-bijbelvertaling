...
GROUND_SHA = "E5AF74760E45916197F263122EDF589B9DDA83FAFBC911B7CBB645775807C9E6"
...
def test_1korinthiers_9_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "1korinthiers" / "9.json")
    review_path = ROOT / "data" / "woordnummers-review" / "1korinthiers-9.json"
    ...
    assert [verse["verse"] for verse in book["verses"]] == list(range(1, 28))
    ...
    assert link_count == 451


def test_1korinthiers_9_publishes_all_451_links_at_atomic_targets():
    chapter = _load(ROOT / "data" / "1korinthiers" / "9.json")
    inline = _load(ROOT / "data" / "woordnummers-inline" / "1korinthiers.json")
    inline_verses = inline["chapters"]["9"]
...
def test_1korinthiers_9_preserves_the_documented_tr_variants():
    chapter = _load(ROOT / "data" / "1korinthiers" / "9.json")
    review = _load(ROOT / "data" / "woordnummers-review" / "1korinthiers-9.json")
    ...
    assert strongs_for(2, "mijn") == ["G1699"]
    assert strongs_for(8, "niet") in (["G3361"], ["G3780"])
    assert strongs_for(15, "iemand") == ["G5100"]
    assert strongs_for(23, "dit") == ["G3778"]
    assert strongs_for(27, "niet enigszins") == ["G3381"]

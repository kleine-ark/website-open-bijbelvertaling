...
EXPECTED_GROUND_SHA = "C86AB43E45DE2EA708B4D1A08225A441F5BD205B1546FFBF7DBB19EACAC5E47A"
...
def test_mattheus_nine_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "mattheus" / "9.json")
    review_path = ROOT / "data" / "woordnummers-review" / "mattheus-9.json"
    ...
    assert len(verses[1]["grondtekst"]) == 20
    assert len(verses[2]["grondtekst"]) == 17
    ...
    assert mapping_count == 658
    assert link_count == 658


def test_mattheus_nine_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "mattheus" / "9.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 658
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 658
    assert not any(

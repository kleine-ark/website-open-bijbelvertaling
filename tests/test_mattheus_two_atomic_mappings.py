...
EXPECTED_GROUND_SHA = "95334ECBC4145C936A84A386CC15AC6EB3ECAE67FC9F6341C1B468B9E774BB55"
...
def test_mattheus_two_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "mattheus" / "2.json")
    review_path = ROOT / "data" / "woordnummers-review" / "mattheus-2.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 24))
    ...
    for record in book["verses"]:
        ...
        for index, mapping in enumerate(mappings):
            ...
            if target:
                ...
                assert mapping["toelichting"]
                anchor = mapping["anker"]
            occurrences = _whole_word_occurrences(verse["text2026"], anchor)
            assert occurrences, (record["verse"], index, anchor)
            occurrence = mapping.get("voorkomen")
            if occurrence is None:
                assert len(occurrences) == 1, (record["verse"], index, anchor, len(occurrences))
                occurrence = 1
            else:
                assert 1 <= occurrence <= len(occurrences)
            strong = verse["grondtekst"][mapping["grondindices"][0]]["strongs"]
            key = (record["verse"], target.casefold(), anchor.casefold(), occurrence, strong)
            assert key not in runtime_keys, key
            runtime_keys.add(key)

    assert mapping_count == 461
    assert link_count == 461
    assert empty_count == 1


def test_mattheus_two_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "mattheus" / "2.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 461
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 461
    assert not any(

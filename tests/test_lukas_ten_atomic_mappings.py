...
EXPECTED_GROUND_SHA = "94546252DB289371F3394431E2F96A24C76DCEF6D41AB1060FEE39C1282FEE3A"
...
def test_lukas_ten_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "lukas" / "10.json")
    review_path = ROOT / "data" / "woordnummers-review" / "lukas-10.json"
    ...
    assert mapping_count == 660
    assert link_count == 803


def test_lukas_ten_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "lukas" / "10.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 660
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 803
    assert not any(

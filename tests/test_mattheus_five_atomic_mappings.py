...
EXPECTED_GROUND_SHA = "30C280BC36E74C3BE77AB6AAF4AC7FC1ED96887A6A22E823091113DA9FBF3187"
...
def test_mattheus_five_review_is_atomic_complete_pinned_and_reachable():
    chapter = _load(ROOT / "data" / "mattheus" / "5.json")
    review_path = ROOT / "data" / "woordnummers-review" / "mattheus-5.json"
    ...
    assert [record["verse"] for record in book["verses"]] == list(range(1, 49))
    ...
    assert mapping_count == 843
    assert link_count == 843


def test_mattheus_five_projected_records_are_atomic_and_complete():
    chapter = _load(ROOT / "data" / "mattheus" / "5.json")
    records = [mapping for verse in chapter["verses"] for mapping in verse["woordnummers"]]

    assert len(records) == 843
    assert sum(len(mapping["herkomst"]["grondindices"]) for mapping in records) == 843
    assert not any(
